...
//zwraca raport o ilosci i lacznej wartosci wszystkich produktow
...
router.post("/products", async (req, res) => {
    try {
        const collection = await db.getDatabase().collection("products");
        const ifExists = await collection
            .find({ nazwa: req.body.nazwa })
            .toArray();
...
router.delete("/products/:id", async (req, res) => {
    try {
        ...
        if (result.deletedCount === 0)
            res.status(400).send("produkt nie zostal usuniety");
